function varargout = mergeActivations(varargin)
% MERGEACTIVATIONS MATLAB code for mergeActivations.fig
%      MERGEACTIVATIONS, by itself, creates a new MERGEACTIVATIONS or raises the existing
%      singleton*.
%
%      H = MERGEACTIVATIONS returns the handle to a new MERGEACTIVATIONS or the handle to
%      the existing singleton*.
%
%      MERGEACTIVATIONS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MERGEACTIVATIONS.M with the given input arguments.
%
%      MERGEACTIVATIONS('Property','Value',...) creates a new MERGEACTIVATIONS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before mergeActivations_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to mergeActivations_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help mergeActivations

% Last Modified by GUIDE v2.5 23-May-2016 23:46:18

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @mergeActivations_OpeningFcn, ...
    'gui_OutputFcn',  @mergeActivations_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before mergeActivations is made visible.
function mergeActivations_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to mergeActivations (see VARARGIN)

% Choose default command line output for mergeActivations
...
% UIWAIT makes mergeActivations wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = mergeActivations_OutputFcn(hObject, eventdata, handles)
...
global subPath
% first we will save the superimpose_list to the subInfo.mat file (if there
...
varargout = mergeActivations_OutputFcn(hObject, eventdata, handles);

files_table = get(handles.files_table, 'Data');
mergeActivationsClinic(subInfo, files_table)
close;
...
if ischar(newSubPath)
    ...
    if exist(fullfile(subPath, 'subInfo.mat'), 'file')
        ...
        mergeActivations_OpeningFcn(hObject, eventdata, handles, subInfo)
    end
end
...
% add them to the current mergeActivations list
...
% remove them from the current mergeActivations list
% find the string and remove it from the mergeActivations list
...
if (curCol == 5)
    ...
    if isequal(files_table{curRow, 1}, '*')
        ...
        if (val == 0)
            ...
            newSeriesName = [newSeriesName '_Merged_Activations'];
...
mergeActivations_OpeningFcn(hObject, eventdata, handles, varargin)
