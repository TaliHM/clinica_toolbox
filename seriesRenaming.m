function varargout = seriesRenaming(varargin)
% seriesRenaming MATLAB code for seriesRenaming.fig
%      seriesRenaming, by itself, creates a new seriesRenaming or raises the existing
%      singleton*.
%
%      H = seriesRenaming returns the handle to a new seriesRenaming or the handle to
%      the existing singleton*.
%
%      seriesRenaming('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in seriesRenaming.M with the given input arguments.
%
%      seriesRenaming('Property','Value',...) creates a new seriesRenaming or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before seriesRenaming_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to seriesRenaming_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help seriesRenaming

% Last Modified by GUIDE v2.5 30-Apr-2017 15:36:13

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @seriesRenaming_OpeningFcn, ...
    'gui_OutputFcn',  @seriesRenaming_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before seriesRenaming is made visible.
function seriesRenaming_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to seriesRenaming (see VARARGIN)

% Choose default command line output for seriesRenaming
...
if length(varargin) == 1
    ...
    if uploadSubFlag && isempty(wholeScanSession)
...
% UIWAIT makes seriesRenaming wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = seriesRenaming_OutputFcn(hObject, eventdata, handles)
...
if ischar(newSubPath)
    ...
    if exist(fullfile(subPath, 'subInfo.mat'), 'file')
        ...
        seriesRenaming_OpeningFcn(hObject, eventdata, handles, subPath)
    end
end
...
if ~isempty(eventdata.Indices)
    ...
    curPath = fullfile(subPath, studyDirName, char(sDirNames(row)));
...
if ~isempty(sDir)
    ...
    if (isempty(mismatch_nErr) && isempty(index_nErr))
        if exist(fullfile(subPath, 'subInfo.mat'), 'file')
            ...
            if isfield(subInfo, 'wholeScanSession')
                if ~isempty(subInfo.wholeScanSession)
                    str = sprintf('Series renaming was already done on this subject\n Do you want to override existing files?');
                    choice = questdlg(str, ...
                        'Series renaming paused', ...
                        'Yes','No', 'No');
...
% --- Otherwise, executes on mouse press in 5 pixel border or over createRawDataFolder.
function createRawDataFolder_ButtonDownFcn(hObject, eventdata, handles)
% hObject    handle to createRawDataFolder (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
