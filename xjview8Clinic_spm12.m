...
% ------------------------Appearance Settings------------------------------------
figurePosition =                    [0.025,   0.050,    0.950,    0.900]; %<--- for clinica
sectionViewPosition =               [0.45,0.45,0.6,0.6];
glassViewAxesPosition =             [0.100,   0.45,    0.005,    0.005]; %[0.000,   0.45,    0.5,    0.5];
LegendPanelPosition =   [0.100,   0.45,    0.4,  0.4];   % 	new  left-up panel

if screenResolution(3) <= 1024
    figurePosition =                    [0.100,   0.040,    0.700,    0.900];    % figure position
    sectionViewPosition =               [0.5,       0.61,   0.45,   0.46];    %the right-up axes
    glassViewAxesPosition =            [0100,   0.600,    0.005,    0.005];   %the left-up axes  [0.000,   0.600,    0.464,    0.400];
    LegendPanelPosition =   [0.100,   0.600,    0.360,  0.300]; % 	new  left-up panel
end
...
controlPanelPosition =              [left,     0.080,  0.450,      0.550];   %the left-down frame  % change
...
infoTextBoxPosition =               stretchMatrix*[0.000,   9*heightUnit,    1,           editBoxHeight*8]' + controlPanelOffset;

% ---->  change - haim
TimeCoursePushPosition =            stretchMatrix*[0.800,   7*heightUnit,    editBoxWidth*1,    editBoxHeight]' + controlPanelOffset;
xClusterCheckPosition =             stretchMatrix*[0.520,   7*heightUnit,    editBoxWidth*1.2,    editBoxHeight]' + controlPanelOffset;%xjViewPosition =                    stretchMatrix*[0.400,   13*heightUnit,    editBoxWidth*2.5,    editBoxHeight*3]' + controlPanelOffset;

sectionViewListboxPosition =        [sectionViewPosition(1)+0.42, sectionViewPosition(2)+0.05, 0.1, 0.14];
...
handles.infoTextBox = uicontrol(handles.figure, 'style','edit',...
    ...
    'max',2, 'min',0);
try
    %     urlread('http://people.hnl.bcm.edu/cuixu/xjView/guestbook/stat.php');
...
% ---> Added by Haim Ben-Ami
handles.TimeCoursePush = uicontrol(handles.figure, 'style', 'push',...
    'unit','normalized',...
    'String', 'Time-Course', ...
    'position', TimeCoursePushPosition,...
    'callback', @CallBack_TimeCoursePush);
handles.xClusterCheck = uicontrol(handles.figure,'style','checkbox',...
    'unit','normalized',...
    'string','Use Cluster' ,...
    'horizontal','left',...
    'position',xClusterCheckPosition,...
    'callback',@CallBack_xClusterCheck);
handles.TimeCourseFigure = '';


rectHeight =  0.2;
rectWidth =  0.5;

LegendPanelPosition =   [0.100,   0.600,    0.360,  0.300];
stretchMatrixPanel =   diag([LegendPanelPosition(1),LegendPanelPosition(2),LegendPanelPosition(3),LegendPanelPosition(4)]);
Rect1Position = stretchMatrixPanel*[0.100,  1.450,    rectWidth,    rectHeight]' ;%+ controlPanelOffset;
Rect2Position = stretchMatrixPanel*[0.100,  1.250,    rectWidth,    rectHeight]' ;%+ controlPanelOffset;
Rect3Position = stretchMatrixPanel*[0.100,  1.050,    rectWidth,    rectHeight]' ;%+ controlPanelOffset;
Rect4Position = stretchMatrixPanel*[0.100,  0.850,    rectWidth,    rectHeight]' ;%+ controlPanelOffset;
Rect5Position = stretchMatrixPanel*[0.100,  0.650,    rectWidth,    rectHeight]' ;%+ controlPanelOffset;
Rect6Position = stretchMatrixPanel*[0.100,  0.450,    rectWidth,    rectHeight]' ;%+ controlPanelOffset;
Rect7Position = stretchMatrixPanel*[0.100,  0.250,    rectWidth,    rectHeight]' ;%+ controlPanelOffset;

Text1Position = stretchMatrixPanel*[2.800,  1.450,    3*rectWidth,    rectHeight]' ;%+ controlPanelOffset;
Text2Position = stretchMatrixPanel*[2.800,  1.250,    3*rectWidth,    rectHeight]' ;%+ controlPanelOffset;
Text3Position = stretchMatrixPanel*[2.800,  1.050,    3*rectWidth,    rectHeight]' ;%+ controlPanelOffset;
Text4Position = stretchMatrixPanel*[2.800,  0.850,    3*rectWidth,    rectHeight]' ;%+ controlPanelOffset;
Text5Position = stretchMatrixPanel*[2.800,  0.650,    3*rectWidth,    rectHeight]' ;%+ controlPanelOffset;
Text6Position = stretchMatrixPanel*[2.800,  0.450,    3*rectWidth,    rectHeight]' ;%+ controlPanelOffset;
Text7Position = stretchMatrixPanel*[2.800,  0.250,    3*rectWidth,    rectHeight]' ;%+ controlPanelOffset;

handles.legendPanel = uipanel('Title','Legend','FontSize',12,...
    'Position',LegendPanelPosition);

handles.rect(1) = uicontrol(handles.legendPanel,'style','text',...
    'unit','normalized',...
    'Position',Rect1Position);

handles.rect(2)= uicontrol(handles.legendPanel,'style','text',...
    'unit','normalized',...
    'Position',Rect2Position);

handles.rect(3) = uicontrol(handles.legendPanel,'style','text',...
    'unit','normalized',...
    'Position',Rect3Position);

handles.rect(4) = uicontrol(handles.legendPanel,'style','text',...
    'unit','normalized',...
    'Position',Rect4Position);

handles.rect(5) = uicontrol(handles.legendPanel,'style','text',...
    'unit','normalized',...
    'Position',Rect5Position);

handles.rect(6) = uicontrol(handles.legendPanel,'style','text',...
    'unit','normalized',...
    'Position',Rect6Position);

handles.rect(7) = uicontrol(handles.legendPanel,'style','text',...
    'unit','normalized',...
    'Position',Rect7Position);

handles.text(1) = uicontrol(handles.legendPanel,'style','text',...
    'unit','normalized',...
    'Position',Text1Position);

handles.text(2) = uicontrol(handles.legendPanel,'style','text',...
    'unit','normalized',...
    'Position',Text2Position);

handles.text(3) = uicontrol(handles.legendPanel,'style','text',...
    'unit','normalized',...
    'Position',Text3Position);

handles.text(4) = uicontrol(handles.legendPanel,'style','text',...
    'unit','normalized',...
    'Position',Text4Position);

handles.text(5) = uicontrol(handles.legendPanel,'style','text',...
    'unit','normalized',...
    'Position',Text5Position);

handles.text(6) = uicontrol(handles.legendPanel,'style','text',...
    'unit','normalized',...
    'Position',Text6Position);

handles.text(7) = uicontrol(handles.legendPanel,'style','text',...
    'unit','normalized',...
    'Position',Text7Position);

%          uicontrol(handles.figure,'style','panel',...
%         'unit','normalized',...
%         'string','Use Cluster' ,...
%         'horizontal','left',...
%         'position',xClusterCheckPosition,...
...
if ~isempty(isEEG)
    ...
    f = [handles.subDetails{5} '_' contrastName];
else
    ...
    f = strrep(f, '(', '');
%     f = [char(f) '_' contrastName];
end

cSize = handles.clusterSizeThreshold;

% removing trailing zeros
p = sprintf('%.5g', handles.pValue);

% taking care if we have format of: 1e-06
eloc = strfind(p, '-');
if ~isempty(eloc)
    zeroCount = str2double(p(eloc+1:end))-1;
    if (zeroCount == 4)
        p = sprintf('%.5f', handles.pValue);
    end
end

% and removing the 0. at the beginning
pval = regexp(p, '[^.]+\d', 'match');
file = fullfile(resultsPath, [char(f) '_c' num2str(cSize) '_p' char(pval)]);
...
for mm = clusterID
    ...
    disp(['Number of voxels: ' num2str(length(pos))])
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Time course drawing - Added by Haim Ben-Ami & small additions by Lonia K.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function CallBack_TimeCoursePush(hObject, eventdata)

xjview8Clinic_spm12_timeCoursePlot(hObject); % modified by THM 08.03.2017
%timeCoursePlot(hObject);

% % TR = 3; % sec
% RegressorColors = [ 'brgcmy' ];
%
% if isempty( handles.file4D ),
%     % load 4D paradigm information
%     tempSeriesNumber = str2num( handles.Series(1:4) );
%     tempSeriesname = dir( fullfile( handles.rootFolder, [ '*Se' num2str( tempSeriesNumber, '%0.2d' ) '*' ] ) );
%     if exist( fullfile( handles.rootFolder, tempSeriesname(1).name, 'paradigm.mat' ) ,'file'),
%         handles.paradigm = load( fullfile( handles.rootFolder, tempSeriesname(1).name, 'paradigm.mat' ) );
%     else
%         warning( [ 'Can''t find paradigm file for ploting onstes: ' fullfile( handles.rootFolder, [ '*Se' num2str( tempSeriesNumber, '%0.2d' ) '*' ] ) ] );
%         set( handles.infoTextBox, 'string', [ 'Can''t find paradigm file for ploting onstes: ' fullfile( handles.rootFolder, [ '*Se' num2str( tempSeriesNumber, '%0.2d' ) '*' ] ) ] );
%         handles.paradigm = '';
%     end
%     guidata( hObject, handles ); % Store GUI data
%
%     % load 4D file
%     temp4Dname = dir( fullfile( handles.subFuncFolder, '4D*.nii' ) );
%     if isempty( temp4Dname ),
%         error( [ 'Can''t find 4D file for time-course at: ' pwd ] );
%     end
%     handles.file4D = load_untouch_nii( fullfile( handles.subFuncFolder, temp4Dname(1).name ) ); % 29.07.2015 THM - used to be load_nii
%     guidata( hObject, handles );
%
%     % load conditions names
%     if exist( fullfile( handles.rootFolder, tempSeriesname(1).name, 'paradigm.mat' ) ,'file'),
%         handles.cond_names = load( fullfile( handles.rootFolder, tempSeriesname(1).name, 'Conds_Names.mat' ) );
%     else
%         warning( [ 'Can''t find paradigm file for ploting onstes: ' fullfile( handles.rootFolder, [ '*Se' num2str( tempSeriesNumber, '%0.2d' ) '*' ] ) ] );
%         set( handles.infoTextBox, 'string', [ 'Can''t find Conds_Names file for legend: ' fullfile( handles.rootFolder, [ '*Se' num2str( tempSeriesNumber, '%0.2d' ) '*' ] ) ] );
%         handles.cond_names = '';
%     end
% end
%
...
% end
%
% % Plot the averaged time-course
% try
%     axes( handles.TimeCourseFigure );
% catch
%     handles.TimeCourseFigure = axes('Parent',gcf,'units','normalized','Position',[0.51, 0.025, 0.47, 0.340],'Visible','off');
%     guidata( hObject, handles );
%     UserData.hReg = [];
...
% hold on;
%
% set(handles.rect(1), 'BackgroundColor', 'k');
% set(handles.text(1), 'String', 'Time Course');
% if ~isempty( handles.paradigm )
%     for regressor = 1:size( handles.paradigm.Conds_and_Onsets, 2 ),
%         set(handles.rect(regressor+1), 'BackgroundColor', RegressorColors(regressor));
%         set(handles.text(regressor+1), 'String', handles.cond_names.Condition_Names(regressor));
%         for scanPoint = 1:size( handles.paradigm.Conds_and_Onsets, 1 ),
%             if( handles.paradigm.Conds_and_Onsets( scanPoint, regressor ) ~= 0 )
%                 if( handles.paradigm.Conds_and_Durations( scanPoint, regressor ) == 0 ),
%                     plot( [handles.paradigm.Conds_and_Onsets( scanPoint, regressor )+DELAY, handles.paradigm.Conds_and_Onsets( scanPoint, regressor )+DELAY], [tempAxis(3), tempAxis(4)], RegressorColors( mod(regressor-1,length(RegressorColors))+1 ) );
%                 else
%                     rectangle( 'Position', [ handles.paradigm.Conds_and_Onsets( scanPoint, regressor )+DELAY, tempAxis(3), handles.paradigm.Conds_and_Durations( scanPoint, regressor ), tempAxis(4)-tempAxis(3) ], 'FaceColor', RegressorColors( mod(regressor-1,length(RegressorColors))+1 ) );
%                 end
%             end
%         end
%     end
...
%
%
% % legend('timecourse', handles.cond_names.Condition_Names(:));
% hold off;
% guidata(hObject, handles);
% return;
...
if isdir(pluginpath)
    ...
    if ~isempty(pluginfiles)
        ...
        for k = 1:length(pluginfiles)
            [p pluginname e] = fileparts(pluginfiles(k).name); % 29.07.2015 THM
...
if strcmp( 'SetCoords', Operation ),
    CallBack_TimeCoursePush( TimeCourseFigure );
end
